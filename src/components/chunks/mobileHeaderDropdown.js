...
export default function MobileHeaderDropdown() {
    ...
    return (
        <li className={'md:hidden block'}>
            <button className={'flex items-center justify-center w-[32px] h-[32px]'} onClick={() => {
                setOpened(true);
            ...
            {
                (isOpened) ? (
                    <div className={'bg-slate-900/80 absolute z-40 top-0 left-0 w-full h-full backdrop-blur-sm p-4 flex justify-end items-start'}>
                        <div className={'w-80 bg-slate-800 border-t-2 border-t-slate-600/30 rounded-md flex flex-col gap-3'}>
                            <ul>
                                <li className={'flex justify-between w-full'}>
                                    ...
                                    <button
                                        className={'p-4 text-slate-400 hover:text-slate-300'}
                                        onClick={() => {
                                            setOpened(false);
                                            document.body.classList.remove('overflow-hidden');
                                        }}>
                                        <IconComponent type={'x'}/>
                                    </button>
                                </li>
